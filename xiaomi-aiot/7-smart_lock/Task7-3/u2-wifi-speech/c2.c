...
uint8_t c2_init(uint8_t zigbee_mode)
{
		...
		uint8_t group_id[5] = {0xFD,0x02,0x09,0x01,0xFF};           //配置网络组
			// uint8_t test_state[4] = {0xFE,0x01,0x17,0xFF};		//test,get working state,expect 0xFB,0X03
		result = 0;
		...
		for(i=0;i<3;i++)
		{
			...
			uart_recv(serial_fd, recvdata, 10);
				//  printf("配置设备类型 recvdata[0] ==%d,recvdata[1] ==%d\n",recvdata[0],recvdata[1]);
		...
		if(result)
		{
			...
			for(i=0;i<3;i++)
			{
				...
				uart_recv(serial_fd, recvdata, 10);
				//  printf("配置PAN_ID recvdata[0] ==%d,recvdata[1] ==%d\n",recvdata[0],recvdata[1]);
		...
		if(result)
		{
			...
			for(i=0;i<3;i++)
			{
				...
				uart_recv(serial_fd, recvdata, 10);
				//  printf("module4 recvdata[0] ==%d,recvdata[1] ==%d\n",recvdata[0],recvdata[1]);
...
void c2_broadcast_data(char *send_data,uint8_t mode)
{
     ...
     len = strlen(send_data);
	   ...
	   uart_send(serial_fd,(uint8_t *)sendbuf,len+4);
}
...
调用描述:  接收zigbee发送数据
**********************************************************************************************/
uint8_t c2_rec_data(uint8_t *rec_data, uint16_t len)
{
     uint8_t recvlen;
