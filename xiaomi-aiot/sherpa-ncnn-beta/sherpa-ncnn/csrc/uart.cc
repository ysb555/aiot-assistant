...
int uart_recv_poll(int fd, uint8_t *data, int datalen) {
    struct pollfd pfd;
    pfd.fd = fd;
...
#ifdef S_TIMEOUT
    pfd.revents = 0;
    int timeout = (10*20/115200+1) * 1000; // 转换为毫秒
    if (poll(&pfd, 1, timeout) <= 0) {
        perror("poll");
        return -1;
    }
#else
    if (poll(&pfd, 1, -1) <= 0) { // -1表示无限等待
        perror("poll");
        return -1;
    }
#endif
...
int uart_recv_nonblock(int fd, uint8_t *data, int datalen) {
    // 设置非阻塞模式
