	...
	while (1)
	{
		...
		if (half_second_flag)
		{
			...
			if (e1_nixie_tube_addr.flag)
			{
				...
				case 1:
					display_temperature();
					e1_rgb_control(e1_rgb_led_addr.periph,e1_rgb_led_addr.addr,30,60,0);
					break;
				case 2:
					e1_rgb_control(e1_rgb_led_addr.periph,e1_rgb_led_addr.addr,0,30,60);
					display_humidity();
					break;
				case 3:
					e1_rgb_control(e1_rgb_led_addr.periph,e1_rgb_led_addr.addr,60,0,30);
					display_sunshine();
					break;
				case 0://switch on
					e1_rgb_control(e1_rgb_led_addr.periph,e1_rgb_led_addr.addr,60,60,60);
					memset(dp, 0, 4 * sizeof(uint8_t));
					e1_digital_display(e1_nixie_tube_addr.periph, e1_nixie_tube_addr.addr, 0, 0, 0, 0, dp);
					break;
				case 4://switch off
					e1_rgb_control(e1_rgb_led_addr.periph,e1_rgb_led_addr.addr,0,0,0);
					memset(dp, 0, 4 * sizeof(uint8_t));
					e1_digital_display(e1_nixie_tube_addr.periph, e1_nixie_tube_addr.addr, 0xf, 0xf, 0xf, 0xf, dp);
				}
			}
			else
			{
...
void TIMER3_IRQHandler(void)
{
	if (timer_interrupt_flag_get(TIMER3, TIMER_INT_FLAG_UP))
	{
		...
		if (data_count++ >= 6600)
		{ // display three kinds data in a update circle
			data_count = 0;
			update_flag = 1;
		}
