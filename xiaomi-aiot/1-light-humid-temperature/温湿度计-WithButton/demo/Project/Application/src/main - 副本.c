...
uint8_t system_state = ON; 
uint8_t current_display = 0;//0:non  1:temp  2:hum  3:ss
uint16_t display_count = 0;     
uint16_t second_flag = 0;
uint8_t display_state = 0;      // 
s2_para s2_value;               // 
float ss_value;                 // 

i2c_addr_def s2_th_addr;        //init temp/hum_addr
i2c_addr_def s2_ss_addr;        //init sunshine_addr
i2c_addr_def e1_nixie_tube_addr;// init tube
i2c_addr_def e1_rgb_led_addr;			//init led
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (key_count >= 700)
        {
            ...
            if (s1_key_addr.flag)
            {
                uint8_t key = s1_key_scan(s1_key_addr.periph, s1_key_addr.addr);
                handle_key_press(key);//set value for system_state & current_display
            }
            else
            {
                sprintf((char *)print_buffer, "S1 not found\r\n");
        ...
        //
        if (system_state == ON && data_count == 0)
        {
        ...
        if (system_state == ON && e1_nixie_tube_addr.flag)
        {
            switch (current_display)
            {
                ...
                case 3: // 
                    ...
                    break;
                default: // 
                    e1_digital_display(e1_nixie_tube_addr.periph, e1_nixie_tube_addr.addr, 0x10, 0x10, 0x10, 0x10, dp);
                    e1_rgb_control(e1_rgb_led_addr.periph, e1_rgb_led_addr.addr, 0, 0, 0); // ?? LED
                    break;
            }
        }
        else if (e1_nixie_tube_addr.flag)
        {
            //OFF, stop displaying
            e1_digital_display(e1_nixie_tube_addr.periph, e1_nixie_tube_addr.addr, 0x10, 0x10, 0x10, 0x10, dp);
...
void handle_key_press(uint8_t key)
{
    if (key == SW1) //temperature
    {
        system_state = ON;
        current_display = 1;
    }
    else if (key == SW2) // humidity
    {
        system_state = ON;
        current_display = 2;
    }
    else if (key == SW3) // sunshine
    {
        system_state = ON;
        current_display = 3;
    }
    else if (key == SW4) // reset
    {
        system_state = OFF;
        current_display = 0;
    }
    else if (key == SW5) // switch on and off
    {
        system_state = (system_state == ON) ? OFF : ON;
        if (system_state == OFF)
            current_display = 0;
    }
}
